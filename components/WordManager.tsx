...
import { Upload, Download, Filter, Settings2, List, Search, Plus, Trash2, CheckSquare, Square, ArrowRight, BookOpen, GraduationCap, CheckCircle, RotateCcw, FileDown, ChevronDown, FileText } from 'lucide-react';
...
export const WordManager: React.FC<WordManagerProps> = ({ 
  ...
  const toggleSelectGroup = (group: WordEntry[]) => {
    ...
    groupIds.forEach(id => isGroupSelected ? newSet.delete(id) : newSet.add(id));
    setSelectedWords(newSet);
  };
  ...
  const handleExport = () => {
     let dataToExport = selectedWords.size > 0 ? entries.filter(e => selectedWords.has(e.id)) : filteredEntries;
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
     ...
     reader.onload = async (event) => {
        try {
           ...
           } else showToast('没有新单词被导入', 'warning');
                 ...
                 <>
                  ...
                  <>
                    ...
                    <div className="relative flex items-center" ref={importDropdownRef}>
                      <div className="flex bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden divide-x divide-slate-200">
                        <button 
                          onClick={triggerImport}
                          className="flex items-center px-4 py-2 text-sm font-bold text-blue-600 hover:bg-blue-50 transition"
                        >
                          <Upload className="w-4 h-4 mr-2" /> 导入单词
                        </button>
                        <button 
                          onClick={() => setIsImportDropdownOpen(!isImportDropdownOpen)}
                          className={`px-2 hover:bg-slate-50 transition ${isImportDropdownOpen ? 'bg-slate-100' : ''}`}
                        >
                          <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${isImportDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                      </div>

                      {/* 下拉菜单 */}
                      {isImportDropdownOpen && (
                        <div className="absolute top-full right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden z-[100] animate-in fade-in slide-in-from-top-2">
                           <div className="p-1.5 space-y-1">
                              <button 
                                onClick={handleDownloadTemplate}
                                className="w-full flex items-center px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition group"
                              >
                              ...
                              <button 
                                ...
                                className="w-full flex items-center px-3 py-2 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg transition group"
                              >
                                <FileText className="w-4 h-4 mr-3 text-slate-400 group-hover:text-blue-500" />
                                <span>构建导入模板</span>
                              </button>
                           </div>
                        </div>
